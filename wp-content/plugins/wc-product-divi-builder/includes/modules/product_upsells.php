...
class ET_Builder_Module_WooPro_Upsells extends ET_Builder_Module {
	...
	function init() {
		$this->name            = esc_html__( 'Woo Product Up-sells', 'wc-product-divi-builder' );
		$this->slug            = 'et_pb_woopro_upsells';
		...
		$this->advanced_options = array(
			'fonts' => array(
				'header' => array(
					...
					'css'      => array(
						'main' => "{$this->main_css_element} .upsells > h2",
		...
		$this->custom_css_options = array(
			'header' => array(
				'label' => esc_html__( 'Header', 'wc-product-divi-builder' ),
				'selector' => "{$this->main_css_element} .upsells > h2",
			),
			'product_title' => array(
				'label' => esc_html__( 'Product Title', 'wc-product-divi-builder' ),
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		if( function_exists( 'is_product' ) && is_product() ){
			add_filter( 'woocommerce_upsell_display_args', array( $this, 'change_pro_count' ), 99 );
			...
			// get the content
			$data = WC_ET_BUILDER::content_buffer( 'woocommerce_upsell_display' );
			...
			$class = 'et_pb_woopro_upsells et_pb_module ';
			...
			if( !empty( $heading_text_orientation ) ){
				...
				ET_Builder_Element::set_style( $function_name, array(
					'selector'    => '%%order_class%% .upsells > h2',
...
new ET_Builder_Module_WooPro_Upsells;